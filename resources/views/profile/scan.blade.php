    ...
    <div class="max-w-4xl mx-auto bg-[#102635] border border-[#00c3b3]/20 rounded-xl shadow-lg p-8">
        ...
        <form method="POST" action="{{ route('scan.run') }}">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                @foreach([
                    'nmap_host_discovery' => 'Nmap Host Discovery',
                    'nmap_basic_port_scan' => 'Basic Port Scan',
                    'os_fingerprinting' => 'OS Fingerprinting',
                    'banner_grabbing' => 'Banner Grabbing',
                    'ssh_weak_config' => 'Weak SSH Configuration Check',
                    'ftp_anonymous' => 'FTP Anonymous Login',
                    'smb_share_scan' => 'SMB Share Scan',
                    'http_headers' => 'HTTP Security Headers',
                    'snmp_scan' => 'SNMP v1/v2 Public Scan',
                    'nmap_nse' => 'Nmap NSE Vulnerability Scripts',
                    'nikto' => 'Nikto Web Scan (Simulated)',
                    'ssl_tls' => 'SSL/TLS Scan',
                    'os_patch' => 'OS Patch Check',
                    'docker' => 'Docker Misconfigurations',
                    'firewall' => 'Firewall Status',
                    'passive_sniff' => 'Passive Network Sniffing',
                    'dns_misconfig' => 'DNS Misconfiguration Check',
                ] as $key => $label)
                
                <label class="flex items-center bg-[#0b1d2a] border border-gray-600 p-3 rounded-lg">
                    <input checked type="checkbox" name="features[]" value="{{ $key }}" class="mr-3">
