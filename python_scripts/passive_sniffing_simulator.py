...
class PassiveSnifferSimulator:
    ...
    def generate_json_report(self):
        """Generate JSON report for PHP integration"""
        total_packets = self.packet_count
        encrypted_count = sum(1 for p in self.sample_packets if p["encrypted"])
        encryption_rate = (encrypted_count / len(self.sample_packets) * 100) if self.sample_packets else 0
        
        # Calculate unencrypted services
        unencrypted_protocols = set()
        for packet in self.sample_packets:
            if not packet["encrypted"]:
                unencrypted_protocols.add(packet["protocol"])
        
        # Count credentials visible (packets with password-like data)
        credentials_visible = 0
        for packet in self.sample_packets:
            if not packet["encrypted"] and any(keyword in packet["data"].lower() 
                                             for keyword in ["password", "pass", "login", "user"]):
                credentials_visible += 1
        
        # Determine risk level
        if credentials_visible >= 3 or encryption_rate < 50:
            risk_level = "High"
        elif len(unencrypted_protocols) > 2 or encryption_rate < 70:
            risk_level = "Medium"
        else:
            risk_level = "Low"
        
        report = {
            "unencrypted_services": len(unencrypted_protocols),
            "exposed_sessions": total_packets // 2,  # Rough estimate
            ...
            "vulnerable_protocols": list(unencrypted_protocols),
            "risk_level": risk_level,
            "total_packets_analyzed": total_packets,
            "protocols_detected": len(self.protocol_stats),
            "educational_insights": [
                f"Network traffic analysis completed with {total_packets} packets",
                f"Encryption coverage: {encryption_rate:.1f}%",
                f"Detected {len(unencrypted_protocols)} unencrypted protocols",
                "Recommend implementing full TLS/HTTPS enforcement",
...
def main():
    ...
    parser.add_argument("--json", action="store_true",
                        help="Output JSON format for PHP integration")
    ...
    try:
        ...
        if not args.json:
            ...
            # Quick simulation for JSON output
            for i in range(min(args.time, len(sniffer.sample_packets))):
                if i < len(sniffer.sample_packets):
                    sniffer.process_packet(sniffer.sample_packets[i])
                time.sleep(0.5)

        # Generate JSON report
        json_report = sniffer.generate_json_report()
        
        # Output JSON if requested
        if args.json:
            print(json.dumps(json_report, indent=2))
        else:
            # Display JSON output for reference
            print("\n" + "=" * 70)
            print("📊 JSON OUTPUT FOR PHP INTEGRATION")
            print("=" * 70)
            print(json.dumps(json_report, indent=2))
            print("=" * 70)
